#!/bin/bash
# Oracle ARM Instance Creator — Peak Hours Edition
# Runs during peak windows: 1:00-5:00, 11:00-13:00 CST (via cron every 5min)
# Time check inside script ensures only fires in correct windows
...
# ====== Time Window Check (CST / UTC+8) ======
# Peak: 1:00-5:00 (Oracle resource reclamation) + 11:00-13:00 (maintenance)
...
if [ "$HOUR" -ge 1 ] && [ "$HOUR" -lt 5 ]; then
    ...
    exit 0  # Silent exit outside peak windows
fi
